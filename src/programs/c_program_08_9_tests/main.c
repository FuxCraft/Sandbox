...
// 2d int grid counter setup (need counter variable set to 0)
void define_2d_int_grid_random(int32_t int_arr_2d_grid[ARR_SIZE][ARR_SIZE], size_t counter)
{
    int32_t temp_arr[ARR_SIZE*ARR_SIZE];
    srand(time(NULL));
    for (size_t i = 0; i < ARR_SIZE; i++)
    {
        for (size_t j = 0; j < ARR_SIZE; j++)
...
int main()
{
    ...
    print_2d_int_grid(int_arr_2d_grid);
